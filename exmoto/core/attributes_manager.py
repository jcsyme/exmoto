...
class AttributesManager:
    """A centralized object for managing inter-sectoral objects, dimensions,
        attributes, variables, and units.
    ...
    def add_index_fields(self,
        ...
        design_id : Union[int, None]
            Value for index AttributesManager.dim_design_id; if None, the index is 
            not added
        future_id : Union[int, None]
            Value for index AttributesManager.dim_future_id; if None, the index is 
            not added
        primary_id : Union[int, None]
            Value for index AttributesManager.dim_primary_id; if None, the index 
            is not added
        region : Union[str, None]
            Value for index AttributesManager.dim_region; if None, the index is 
            not added
        strategy_id : Union[int, None]
            Value for index AttributesManager.dim_strategy_id; if None, the index 
            is not added
        time_period : Union[int, None]
            Value for index AttributesManager.dim_time_period; if None, the index 
            is not added
        time_series_id : Union[int, None]
            Value for index AttributesManager.dim_time_series_id; if None, the 
            index is not added
    ...
    def extract_model_variable(self,
        ...
        """Extract an array or data frame of input variables. If 
            return_type == "array_units_corrected", then AttributesManager will 
        ...
        return_type : str
            ...
            * "array_base": np.ndarray of raw values
        throw_error_on_missing_fields : bool
            * True:         Throw an error if the fields associated with modvar 
                            are not found in df_in.
            * False:        Returns None if fields implied by modvar are not 
                            found in df_in
    ...
    def filter_keys_by_attribute(self,
    ...
    def get_baseline_scenario_id(self,
        ...
        dim : str
            ...
            (attribute_dim_####.csv) within the AttributesManager class
        ...
        if field_check not in attr.table.columns:
            ...
            if len(tab_red) > 1:
                ...
                '{tab_red.fp_table}'. Defaulting to minimum value of {ret}.
                """
                warnings.warn(msg)
    ...
    def get_category_replacement_field_dict(self,
        ...
        """Replace categories with the target field associated with the 
            ModelVariable `modvar`. Returns None if the variable is not 
    ...
    def get_subsector_attribute(self, #FIXED
        ...
        if return_type in ["key_varreqs_all", "key_varreqs_partial"]:
            msg = f"""ERROR in get_subector_attribute: invalid key {return_type}--this key 
    ...
    def get_time_periods(self, #FIXED
        ...
        """Get all time periods defined in the time period attribute table. 
            Returns a tuple of the form (time_periods, n), where:
    ...
    def get_valid_categories(self, 
        ...
        ------------------
        categories : Union[List[str], str]
            List of categories to check. If None, returns all valid categories 
            in subsector
        subsector: str
            Subsector to check categories against. If not a valid subsector, 
            returns None. 
        """
    ...
    def get_valid_categories_dict(self, 
        ...
        ------------------
        dict_cats : 
            Dictionary with keys or values to check. If by_value is True, 
            verifies pairs with valid values; otherwise, keeps pairs with valid 
            keys
        subsector : str
            Subsector to check categories against. If not a valid subsector, 
            returns None. 
        ...
        -----------------
        by_value : bool
            Set to True to filter on values instead of keys
        """
    ...
    def merge_array_var_partial_cat_to_array_all_cats(self, #FIXED
        ...
        missing_vals : Union[float, None]
            Optional values to set for categories not in array_vals. If None,
            uses modvar.default_value if modvar is available OR 0.0 if no modvar
            is specified. 
        output_cats: Union[list, None]
            Optional vector of categories associated with the output variable. 
        ...
        output_subsec : Union[str, None]
            Optional output subsector. Default is None. Only used if modvar is 
            None. The combination of output_cats + output_subsec provide a 
            manual override to the modvar option.
        """
    ...
    def reduce_all_cats_array_to_partial_cat_array(self, #FIXED
        ...
        array_vals : np.ndarray
            Input array of data with column categories
        modvar : Union[str, mv.ModelVariable]
            The variable associated with the desired *output* array. This is 
            used to identify which categories should be selected.
        """
    ...
    def rescale_fields_to_target(self,
        ...
        dict_units_source_multiplier : Dict[str, int]
            Map of units to multiplier effect. Takes form:
                ...
                {
                    AttributesManager_UNIT: (unit_name, exponent,)
                    ...
                }
    ...
    def check_projection_input_df(self,
        ...
        if len(fields_dims_notime) > 0:
            ...
            if len(df_fields_dims_notime) > 1:
                msg = f"""Error in project: the input data frame contains multiple 
    ...
    def transfer_df_variables(self,
        ...
        """
        Transfar ModelVariables from source data frame to target data 
            frame.
            ...
            for i, df in dfg:
                ...
                if var_transitions in simplex_modvars:
                    warnings.warn(
                        ...
                        implemeted in AttributesManager.get_variable_to_simplex_group_dictionary() 
...
def is_model_attributes(
    ...
    """
    check if obj is a AttributesManager object
    """
